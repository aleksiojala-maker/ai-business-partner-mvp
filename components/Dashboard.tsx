...
import { Briefcase, Gavel, Calculator, BadgeDollarSign, Sparkles } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ state, onDeletePrompt, onToggleFavorite }) => {
  ...
  const getPromptsForRole = () => {
    let categoryFilter = '';
    switch (selectedRole) {
      case VirtualRole.LAKIMIES: categoryFilter = 'Legal'; break;
      case VirtualRole.TALOUS: categoryFilter = 'Admin'; break;
      case VirtualRole.MYYNTI: categoryFilter = 'Sales'; break;
      default: return state.prompts.filter(p => p.isFavorite); // General shows favorites
    }
    return state.prompts.filter(p => p.category === categoryFilter);
  };
    ...
    <button
      ...
      className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-200 ${
        selectedRole === role 
          ...
          : 'bg-white border-gray-200 text-gray-500 hover:border-goodi-teal hover:text-goodi-teal'
      }`}
    >
